...
exports.calculateTotalDays = exports.extractTimeRange = exports.extractMonthRange = void 0;
 ...
 */
const extractMonthRange = (prompt) => {
    const now = new Date();
...
exports.extractMonthRange = extractMonthRange;
/**
 * ⚡ Regex detection nhanh cho các mẫu thời gian phổ biến
 */
const extractTimeRangeRegex = (question) => {
 ...
 */
const extractTimeRangeLLM = async (question) => {
    const prompt = `
...
Câu hỏi: "${question}"
`;
    const raw = await (0, geminiService_1.askGemini)(prompt);
    try {
 ...
 */
const extractTimeRange = async (question) => {
    // 1️⃣ Regex nhanh
    const regexResult = extractTimeRangeRegex(question);
    if (regexResult)
        return regexResult;
    // 2️⃣ Xử lý tháng / quý / năm bằng extractMonthRange
    const monthRange = (0, exports.extractMonthRange)(question);
    if (monthRange) {
        const { startMonth, startYear, endMonth, endYear } = monthRange;
        const startDate = (0, dayjs_1.default)(`${startYear}-${String(startMonth).padStart(2, "0")}-01`);
        const endDate = (0, dayjs_1.default)(`${endYear}-${String(endMonth).padStart(2, "0")}-01`).endOf("month");
        return {
            type: "relative",
            language: "vi",
